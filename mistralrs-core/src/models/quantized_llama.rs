...
impl ModelWeights {
    ...
    /// Forward pass starting from pre-computed hidden states at a specific layer.
    /// Used by remote workers that only compute a subset of layers.
    /// Returns the hidden states after `end_layer` (inclusive), on CPU.
